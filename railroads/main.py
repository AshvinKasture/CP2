...
Connection = namedtuple(
    "Connection", ['departure', 'arrival', 't_departure', 't_arrival', 'uid'])
...
def read_scenario():
    ...
    cities = [input() for _ in range(ncities)]
...
def build_time_exp_adj(city_connections, connections):
    ...
    for c in connections:
        adjList[con_start_id(c)].append(
            (con_end_id(c), c.t_arrival-c.t_departure, c))
...
def find_schedule(cities, trains, src_city, dst_city, start_time):
    ...
    # Outgoing connections from src_city
    start_con = city_connections[src_city, DEPARTURE]
    # Incoming connection to dst_city
    ...
    start_city_departures = [(con_start_id(c), c.t_departure)
                             for c in start_con]
